# -*- coding: utf-8 -*-
"""Builds the KCC2026 attention-mechanism (MHA/GQA/MLA) + KV cache footprint notebook."""
import json
...
# ===========================================================================
md(r"""# Attention 메커니즘 비교: MHA vs GQA vs MLA — Attention Map + KV Cache Footprint
...
code(r"""# 1-1. 직관 그림: Query head 는 많아도, 저장하는 KV head 수가 줄어들면 캐시가 작아진다.
...
# --- Section 2 ---
md(r"""## 2. ⚠️ 입력 데이터: `hash_ids` 는 token id 가 아니다
...
code(r"""# 2-1. 데모 입력: 앞부분이 반복되는 문장 (prefix 공유 모사)
...
# --- Section 3 ---
md(r"""## 3. 공통 헬퍼: 모델 로드 · 어텐션 추출 · 시각화

모델은 **한 번에 하나만** 올리고, 끝나면 GPU 메모리를 비웁니다(셋을 동시에 올리지 않음).""")

code(r'''# 3-1. 로드 / 어텐션 추출 / 메모리 해제
...
code(r'''# 3-2. head grid 시각화 (한 레이어의 여러 head 를 나란히)
...
code(r"""# 5-1. GQA: Qwen2.5-1.5B-Instruct
...
code(r"""# 6-1. MLA: DeepSeek-V2-Lite  (A100 권장)
try:
    free(model_gqa)
...
# --- Section 7: footprint ---
md(r"""## 7. KV Cache Footprint — 토큰당 메모리와 컨텍스트 폭발
...
code(r"""# 7-1. 동일 7B급 레퍼런스 config 로 apples-to-apples 비교
...
code(r"""# 7-3. (참고) 실제 로드한 모델 config 에서 직접 계산
...
out="kvcache_attention_mha_gqa_mla.ipynb"
