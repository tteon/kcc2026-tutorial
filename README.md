...
JupyterLab이 `0.0.0.0`에 bind되고, 고정 토큰 `kcc2026-tutorial`로 열린다. 접속 URL은 터미널에 출력된다.
로컬에서는 `http://localhost:8888/lab?token=kcc2026-tutorial`, 원격 서버에서는 `http://<server-ip>:8888/lab?token=kcc2026-tutorial` 형식으로 접속한다. 8888이 사용 중이면 8889 등 다음 포트가 사용된다.
...
- 고정 토큰 `kcc2026-tutorial`로 JupyterLab 실행 (`--ip=0.0.0.0`)
